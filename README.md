...
- **連想プリフェッチ（opt-in）**: `ch issue list` を起点に、列挙された各 issue の `gh issue view` を `chd` が裏で先読みする。後続の `ch issue view <N>` がキャッシュヒットになる。`prefetch = true` で有効化
...
`prefetch = true`（または `CH_PREFETCH=1`）のとき、`ch issue list` が走るたびに
`chd` がそのリストの issue 番号を `gh issue list --json number` で取り直し、各
`gh issue view <N>` を裏で実行してキャッシュを温める。次に `ch issue view <N>` を
叩くと即ヒットする。

- 1 回の list につき最大 20 件まで。`--state` / `--label` などの絞り込みは引き継ぐ
- 既に fresh な `issue_view` は叩き直さないので、TTL 内に `issue list` を連打しても安い
- `gh` が非ゼロで終わった `issue view` はキャッシュしない（rate limit の 403 を焼き付けない）
- GitHub API のレート制限を考慮して既定は off
...
| `prefetch` / `CH_PREFETCH` | `false` | true なら `ch issue list` 後に列挙された各 issue view を `chd` が先読みする |
